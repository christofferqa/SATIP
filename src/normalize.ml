...
let normalize_function func =
  ...
  let return_id =
    match stm.Ast.stm with
    | Ast.Return id -> id
    | _ -> Error.phase "Normalize" "Internal error." in
  
  let func_decl_desc =
    { NAst.function_name    = func.EAst.function_decl.EAst.function_name;
      NAst.function_formals = func.EAst.function_decl.EAst.function_formals;
      NAst.function_body    = body;
      NAst.function_env     = env;
      NAst.function_return  = return_id } in
  { NAst.function_decl_pos = func.EAst.function_decl_pos;
    NAst.function_decl = func_decl_desc }
...
let normalize_program prog =
  { NAst.program_name = prog.EAst.program_name;
    NAst.program_decl = normalize_functions prog.EAst.program_decl }