...
let rec normalize_exp exp stms env depth =
  ...
  | Ast.PointerInvocation (exp', exps) ->
    ...
    let stm_temp_assign = { Ast.stm_pos = Lexing.dummy_pos; Ast.stm = Ast.VarAssignment (temp_id, normalized_exp'); Ast.stm_id = Utils.new_id !(Utils.id) } in
    let env = Env.add temp_id.Ast.identifier (LocalDecl stm_temp_decl) env in
        
    ({ exp with Ast.exp = Ast.FunctionInvocation (temp_id, normalized_exps) }, stm_temp_decl :: stm_temp_assign :: stms, env)
  | Ast.Malloc ->
    if depth = 0 then
      (exp, stms, env)
    else
...
let rec normalize_stm stm env stms =
  ...
  | Ast.Return exp ->
    ...
    | _ ->
      ...
      let stm_return = { stm with Ast.stm = Ast.Return exp_id } in
      
      (new_stms @ stm_temp_decl :: stm_temp_assign :: stm_return :: stms, env))
  | Ast.While (exp, stms') ->
    let (normalized_exp, new_stms, env) = normalize_exp exp [] env 0 in
  ...
  | Ast.PointerAssignment (exp1, exp2) ->
    ...
    | Ast.Unop (Ast.Dereference, _), _ ->
      ...
      let exp_id = { Ast.exp_pos = Lexing.dummy_pos; Ast.exp = Ast.Identifier temp_id; Ast.exp_id = Utils.new_id !(Utils.id) } in
      let stm_temp_decl = { Ast.stm_pos = Lexing.dummy_pos; Ast.stm = Ast.LocalDecl [temp_id]; Ast.stm_id = Utils.new_id !(Utils.id) } in
      let stm_temp_assign = { Ast.stm_pos = Lexing.dummy_pos; Ast.stm = Ast.VarAssignment (temp_id, normalized_exp2); Ast.stm_id = Utils.new_id !(Utils.id) } in
      let env = Env.add temp_id.Ast.identifier (LocalDecl stm_temp_decl) env in
